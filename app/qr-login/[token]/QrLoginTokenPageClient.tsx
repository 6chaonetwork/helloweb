  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.08),transparent_24%),linear-gradient(180deg,#050816,#090d18)] px-6 py-10 text-white">
      <div className="mx-auto max-w-xl">
        <Card className="rounded-[32px] border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.03))]">
          <CardContent className="p-8">
            <p className="m-0 text-xs uppercase tracking-[0.28em] text-white/42">HelloClaw QR Login</p>
            <h1 className="mt-5 text-4xl leading-tight font-semibold">确认这台设备的登录请求</h1>
            <p className="mt-4 text-base leading-8 text-white/68">
              这是 HelloClaw 桌面端发起的扫码登录确认页。确认后，桌面客户端会继续完成连接。
            </p>

            <div className="mt-8 rounded-[24px] border border-border bg-secondary p-5">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Button type="button" onClick={approve} disabled={submitting || loading || status === "APPROVED"} size="lg">
                {submitting ? "确认中..." : status === "APPROVED" ? "已确认" : "确认登录这台设备"}
              </Button>
              <Button type="button" variant="outline" onClick={() => void loadStatus()} disabled={loading} size="lg">
                刷新状态
              </Button>
            </div>
          </CardContent>
