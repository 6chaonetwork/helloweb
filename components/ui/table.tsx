...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn("border-b border-border transition-colors hover:bg-background/35 data-[state=selected]:bg-background/45", className)}
      {...props}
    />
  );
}
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn("h-11 px-4 text-left align-middle text-xs font-medium tracking-[0.22em] uppercase text-muted-foreground", className)}
      {...props}
    />
  );
}
